...
export default function CandidatePicker({
  ...
  return (
    <section className={`candidate-section ${loading ? "is-loading" : ""}`}>
      <div className="section-heading">
        <h2>Choose the card</h2>
        <p>
          {loading
            ? "Peeking prices for the selected card…"
            : payload.candidates.length > 1
              ? "Select the exact card before peeking the prices."
              : "Did you mean one of these cards?"}
        </p>
        {pagination && !loading && (
          <p className="candidate-count">
            {pagination.total} possible cards · page {page + 1} of {totalPages}
          </p>
        )}
      </div>
